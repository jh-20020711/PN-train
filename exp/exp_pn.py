...
class Exp(Exp_Basic):
    ...
    def _loop_pattern_CV(self, dataloader, current_type='holiday'):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(dataloader)):
            ...
            # 计算并存储属性分数
            self.set_important_score_CV(t_activations, s_activations, t_activations_save, s_activations_save, current_type, index=i)
        ...
        # 时间层 holiday
        self.q_t_mean_ho, self.q_t_var_ho, self.q_t_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.k_t_mean_ho, self.k_t_var_ho, self.k_t_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.v_t_mean_ho, self.v_t_var_ho, self.v_t_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.out_t_mean_ho, self.out_t_var_ho, self.out_t_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.up_t_mean_ho, self.up_t_var_ho, self.up_t_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.down_t_mean_ho, self.down_t_var_ho, self.down_t_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers

        # 空间层 holiday
        self.q_s_mean_ho, self.q_s_var_ho, self.q_s_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.k_s_mean_ho, self.k_s_var_ho, self.k_s_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.v_s_mean_ho, self.v_s_var_ho, self.v_s_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.out_s_mean_ho, self.out_s_var_ho, self.out_s_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.up_s_mean_ho, self.up_s_var_ho, self.up_s_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.down_s_mean_ho, self.down_s_var_ho, self.down_s_cv_ho = [None]*num_layers, [None]*num_layers, [None]*num_layers

        # 时间层 general
        self.q_t_mean_ge, self.q_t_var_ge, self.q_t_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.k_t_mean_ge, self.k_t_var_ge, self.k_t_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.v_t_mean_ge, self.v_t_var_ge, self.v_t_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.out_t_mean_ge, self.out_t_var_ge, self.out_t_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.up_t_mean_ge, self.up_t_var_ge, self.up_t_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.down_t_mean_ge, self.down_t_var_ge, self.down_t_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers

        # 空间层 general
        self.q_s_mean_ge, self.q_s_var_ge, self.q_s_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.k_s_mean_ge, self.k_s_var_ge, self.k_s_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.v_s_mean_ge, self.v_s_var_ge, self.v_s_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.out_s_mean_ge, self.out_s_var_ge, self.out_s_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.up_s_mean_ge, self.up_s_var_ge, self.up_s_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers
        self.down_s_mean_ge, self.down_s_var_ge, self.down_s_cv_ge = [None]*num_layers, [None]*num_layers, [None]*num_layers

        #调用函数填充统计量
        if current_type == 'holiday':
            self.q_t_mean_ho, self.q_t_var_ho, self.q_t_cv_ho = self.compute_stats_from_activations(self.q_imp_t_all_ho, num_layers)
            self.k_t_mean_ho, self.k_t_var_ho, self.k_t_cv_ho = self.compute_stats_from_activations(self.k_imp_t_all_ho, num_layers)
            self.v_t_mean_ho, self.v_t_var_ho, self.v_t_cv_ho = self.compute_stats_from_activations(self.v_imp_t_all_ho, num_layers)
            self.out_t_mean_ho, self.out_t_var_ho, self.out_t_cv_ho = self.compute_stats_from_activations(self.out_imp_t_all_ho, num_layers)
            self.up_t_mean_ho, self.up_t_var_ho, self.up_t_cv_ho = self.compute_stats_from_activations(self.up_imp_t_all_ho, num_layers)
            self.down_t_mean_ho, self.down_t_var_ho, self.down_t_cv_ho = self.compute_stats_from_activations(self.down_imp_t_all_ho, num_layers)

            self.q_s_mean_ho, self.q_s_var_ho, self.q_s_cv_ho = self.compute_stats_from_activations(self.q_imp_s_all_ho, num_layers)
            self.k_s_mean_ho, self.k_s_var_ho, self.k_s_cv_ho = self.compute_stats_from_activations(self.k_imp_s_all_ho, num_layers)
            self.v_s_mean_ho, self.v_s_var_ho, self.v_s_cv_ho = self.compute_stats_from_activations(self.v_imp_s_all_ho, num_layers)
            self.out_s_mean_ho, self.out_s_var_ho, self.out_s_cv_ho = self.compute_stats_from_activations(self.out_imp_s_all_ho, num_layers)
            self.up_s_mean_ho, self.up_s_var_ho, self.up_s_cv_ho = self.compute_stats_from_activations(self.up_imp_s_all_ho, num_layers)
            self.down_s_mean_ho, self.down_s_var_ho, self.down_s_cv_ho = self.compute_stats_from_activations(self.down_imp_s_all_ho, num_layers)

            self.mean_all_q_t_ho = self.compute_global_mean(self.q_imp_t_all_ho, num_layers)
            self.mean_all_k_t_ho = self.compute_global_mean(self.k_imp_t_all_ho, num_layers)
            self.mean_all_v_t_ho = self.compute_global_mean(self.v_imp_t_all_ho, num_layers)
            self.mean_all_out_t_ho = self.compute_global_mean(self.out_imp_t_all_ho, num_layers)
            self.mean_all_up_t_ho = self.compute_global_mean(self.up_imp_t_all_ho, num_layers)
            self.mean_all_down_t_ho = self.compute_global_mean(self.down_imp_t_all_ho, num_layers)
            self.mean_all_q_s_ho = self.compute_global_mean(self.q_imp_s_all_ho, num_layers)
            self.mean_all_k_s_ho = self.compute_global_mean(self.k_imp_s_all_ho, num_layers)
            self.mean_all_v_s_ho = self.compute_global_mean(self.v_imp_s_all_ho, num_layers)
            self.mean_all_out_s_ho = self.compute_global_mean(self.out_imp_s_all_ho, num_layers)
            self.mean_all_up_s_ho = self.compute_global_mean(self.up_imp_s_all_ho, num_layers)
            self.mean_all_down_s_ho = self.compute_global_mean(self.down_imp_s_all_ho, num_layers)

        elif current_type == 'general':
            self.q_t_mean_ge, self.q_t_var_ge, self.q_t_cv_ge = self.compute_stats_from_activations(self.q_imp_t_all_ge, num_layers)
            self.k_t_mean_ge, self.k_t_var_ge, self.k_t_cv_ge = self.compute_stats_from_activations(self.k_imp_t_all_ge, num_layers)
            self.v_t_mean_ge, self.v_t_var_ge, self.v_t_cv_ge = self.compute_stats_from_activations(self.v_imp_t_all_ge, num_layers)
            self.out_t_mean_ge, self.out_t_var_ge, self.out_t_cv_ge = self.compute_stats_from_activations(self.out_imp_t_all_ge, num_layers)
            self.up_t_mean_ge, self.up_t_var_ge, self.up_t_cv_ge = self.compute_stats_from_activations(self.up_imp_t_all_ge, num_layers)
            self.down_t_mean_ge, self.down_t_var_ge, self.down_t_cv_ge = self.compute_stats_from_activations(self.down_imp_t_all_ge, num_layers)

            self.q_s_mean_ge, self.q_s_var_ge, self.q_s_cv_ge = self.compute_stats_from_activations(self.q_imp_s_all_ge, num_layers)
            self.k_s_mean_ge, self.k_s_var_ge, self.k_s_cv_ge = self.compute_stats_from_activations(self.k_imp_s_all_ge, num_layers)
            self.v_s_mean_ge, self.v_s_var_ge, self.v_s_cv_ge = self.compute_stats_from_activations(self.v_imp_s_all_ge, num_layers)
            self.out_s_mean_ge, self.out_s_var_ge, self.out_s_cv_ge = self.compute_stats_from_activations(self.out_imp_s_all_ge, num_layers)
            self.up_s_mean_ge, self.up_s_var_ge, self.up_s_cv_ge = self.compute_stats_from_activations(self.up_imp_s_all_ge, num_layers)
            self.down_s_mean_ge, self.down_s_var_ge, self.down_s_cv_ge = self.compute_stats_from_activations(self.down_imp_s_all_ge, num_layers)

            self.mean_all_q_t_ge = self.compute_global_mean(self.q_imp_t_all_ge, num_layers)
            self.mean_all_k_t_ge = self.compute_global_mean(self.k_imp_t_all_ge, num_layers)
            self.mean_all_v_t_ge = self.compute_global_mean(self.v_imp_t_all_ge, num_layers)
            self.mean_all_out_t_ge = self.compute_global_mean(self.out_imp_t_all_ge, num_layers)
            self.mean_all_up_t_ge = self.compute_global_mean(self.up_imp_t_all_ge, num_layers)
            self.mean_all_down_t_ge = self.compute_global_mean(self.down_imp_t_all_ge, num_layers)
            self.mean_all_q_s_ge = self.compute_global_mean(self.q_imp_s_all_ge, num_layers)
            self.mean_all_k_s_ge = self.compute_global_mean(self.k_imp_s_all_ge, num_layers)
            self.mean_all_v_s_ge = self.compute_global_mean(self.v_imp_s_all_ge, num_layers)
            self.mean_all_out_s_ge = self.compute_global_mean(self.out_imp_s_all_ge, num_layers)
            self.mean_all_up_s_ge = self.compute_global_mean(self.up_imp_s_all_ge, num_layers)
            self.mean_all_down_s_ge = self.compute_global_mean(self.down_imp_s_all_ge, num_layers)

        if current_type == 'holiday':
            self.q_t_index_ho = [self.filter_neurons(self.q_t_mean_ho[l], self.q_t_cv_ho[l], self.mean_all_q_t_ho)for l in range(num_layers)]
            self.k_t_index_ho = [self.filter_neurons(self.k_t_mean_ho[l], self.k_t_cv_ho[l], self.mean_all_k_t_ho)for l in range(num_layers)]
            self.v_t_index_ho = [self.filter_neurons(self.v_t_mean_ho[l], self.v_t_cv_ho[l], self.mean_all_v_t_ho)for l in range(num_layers)]
            self.out_t_index_ho = [self.filter_neurons(self.out_t_mean_ho[l], self.out_t_cv_ho[l], self.mean_all_out_t_ho)for l in range(num_layers)]
            self.up_t_index_ho = [self.filter_neurons(self.up_t_mean_ho[l], self.up_t_cv_ho[l], self.mean_all_up_t_ho)for l in range(num_layers)]
            self.down_t_index_ho = [self.filter_neurons(self.down_t_mean_ho[l], self.down_t_cv_ho[l], self.mean_all_down_t_ho)for l in range(num_layers)]
            self.q_s_index_ho = [self.filter_neurons(self.q_s_mean_ho[l], self.q_s_cv_ho[l], self.mean_all_q_s_ho)for l in range(num_layers)]
            self.k_s_index_ho = [self.filter_neurons(self.k_s_mean_ho[l], self.k_s_cv_ho[l], self.mean_all_k_s_ho)for l in range(num_layers)]
            self.v_s_index_ho = [self.filter_neurons(self.v_s_mean_ho[l], self.v_s_cv_ho[l], self.mean_all_v_s_ho)for l in range(num_layers)]
            self.out_s_index_ho = [self.filter_neurons(self.out_s_mean_ho[l], self.out_s_cv_ho[l], self.mean_all_out_s_ho)for l in range(num_layers)]
            self.up_s_index_ho = [self.filter_neurons(self.up_s_mean_ho[l], self.up_s_cv_ho[l], self.mean_all_up_s_ho)for l in range(num_layers)]
            self.down_s_index_ho = [self.filter_neurons(self.down_s_mean_ho[l], self.down_s_cv_ho[l], self.mean_all_down_s_ho)for l in range(num_layers)]

        elif current_type == 'general':
            self.q_t_index_ge = [self.filter_neurons(self.q_t_mean_ge[l], self.q_t_cv_ge[l], self.mean_all_q_t_ge)for l in range(num_layers)]
            self.k_t_index_ge = [self.filter_neurons(self.k_t_mean_ge[l], self.k_t_cv_ge[l], self.mean_all_k_t_ge)for l in range(num_layers)]
            self.v_t_index_ge = [self.filter_neurons(self.v_t_mean_ge[l], self.v_t_cv_ge[l], self.mean_all_v_t_ge)for l in range(num_layers)]
            self.out_t_index_ge = [self.filter_neurons(self.out_t_mean_ge[l], self.out_t_cv_ge[l], self.mean_all_out_t_ge)for l in range(num_layers)]
            self.up_t_index_ge = [self.filter_neurons(self.up_t_mean_ge[l], self.up_t_cv_ge[l], self.mean_all_up_t_ge)for l in range(num_layers)]
            self.down_t_index_ge = [self.filter_neurons(self.down_t_mean_ge[l], self.down_t_cv_ge[l], self.mean_all_down_t_ge)for l in range(num_layers)]
            self.q_s_index_ge = [self.filter_neurons(self.q_s_mean_ge[l], self.q_s_cv_ge[l], self.mean_all_q_s_ge)for l in range(num_layers)]
            self.k_s_index_ge = [self.filter_neurons(self.k_s_mean_ge[l], self.k_s_cv_ge[l], self.mean_all_k_s_ge)for l in range(num_layers)]
            self.v_s_index_ge = [self.filter_neurons(self.v_s_mean_ge[l], self.v_s_cv_ge[l], self.mean_all_v_s_ge)for l in range(num_layers)]
            self.out_s_index_ge = [self.filter_neurons(self.out_s_mean_ge[l], self.out_s_cv_ge[l], self.mean_all_out_s_ge)for l in range(num_layers)]
    ...
    def compute_global_mean(self,activation_list_all, num_layers):
        """
        activation_list_all: 所有层、所有样本的激活值列表
                            形状: [[layer0, layer1, ...], [layer0, layer1, ...], ...]
        num_layers: 模型层数
        return: float, 全局均值
        """
        flat_all = []
        for sample in activation_list_all:          # 遍历每个样本
            for layer in range(num_layers):         # 遍历每层
                flat_all.append(sample[layer])      # 取该层激活值
        flat_all = np.concatenate(flat_all)         # 拼成一维
        return float(flat_all.mean())

    def compute_stats_from_activations(self,activation_list, num_layers):
        mean_list = [None] * num_layers
        var_list = [None] * num_layers
        cv_list = [None] * num_layers

        for layer in range(num_layers):
            layer_activations = [sample[layer] for sample in activation_list]
            layer_matrix = np.stack(layer_activations, axis=0)
            mean = np.mean(layer_matrix, axis=0)
            var = np.var(layer_matrix, axis=0)
            cv = np.divide(np.sqrt(var), mean, out=np.zeros_like(mean), where=mean!=0)
            mean_list[layer] = mean
            var_list[layer] = var
            cv_list[layer] = cv

        return mean_list, var_list, cv_list
    ...
    def detect_CV(self, setting, logger):
        ...
        # 初始化存储列表
        self.q_imp_t_all_ho, self.k_imp_t_all_ho, self.v_imp_t_all_ho, self.out_imp_t_all_ho = [], [], [], []
        self.up_imp_t_all_ho, self.down_imp_t_all_ho = [], []

        self.q_imp_s_all_ho, self.k_imp_s_all_ho, self.v_imp_s_all_ho, self.out_imp_s_all_ho = [], [], [], []
        self.up_imp_s_all_ho, self.down_imp_s_all_ho = [], []

        self.q_imp_t_all_ge, self.k_imp_t_all_ge, self.v_imp_t_all_ge, self.out_imp_t_all_ge = [], [], [], []
        self.up_imp_t_all_ge, self.down_imp_t_all_ge = [], []

        self.q_imp_s_all_ge, self.k_imp_s_all_ge, self.v_imp_s_all_ge, self.out_imp_s_all_ge = [], [], [], []
        self.up_imp_s_all_ge, self.down_imp_s_all_ge = [], []

        # 对节假日数据计算属性分数
        self._loop_pattern_CV(self.indication_loader_ho, current_type='holiday')
        # 对常规数据计算属性分数
        self._loop_pattern_CV(self.indication_loader_ge, current_type='general')
    ...
    def set_important_score_CV(self, t_param, s_param, t_param_save, s_param_save, current_type, index=None):
        ...
        attn_neurons_len = int(self.model.model_dim * self.args.select_ratio_CV)
        up_len = int(self.model.feed_forward_dim * self.args.select_ratio_CV)
        ...
        for name, param in self.model.named_parameters():
            ...
            if match:
                ...
                # 时间层参数处理
                if type == 't':
                    q_imp_t, k_imp_t, v_imp_t, out_imp_t, up_imp_t, down_imp_t = self._get_important_score_CV(t_q, t_k,t_v, t_out,t_up, t_down,layer, name,param,q_imp_t,k_imp_t,v_imp_t,out_imp_t,up_imp_t,down_imp_t)
                # 空间层参数处理
                elif type == 's':
                    q_imp_s, k_imp_s, v_imp_s, out_imp_s, up_imp_s, down_imp_s = self._get_important_score_CV(s_q, s_k,s_v, s_out,s_up, s_down,layer, name,param,q_imp_s,k_imp_s,v_imp_s,out_imp_s,up_imp_s,down_imp_s)
        
        # 按数据类型存储结果
        if current_type == 'holiday':
            self.q_imp_t_all_ho.append(q_imp_t) # 存储节假日Q权重重要性
            self.k_imp_t_all_ho.append(k_imp_t) # 存储节假日K权重重要性
            self.v_imp_t_all_ho.append(v_imp_t)
            self.out_imp_t_all_ho.append(out_imp_t)
            self.up_imp_t_all_ho.append(up_imp_t)
            self.down_imp_t_all_ho.append(down_imp_t)

            self.q_imp_s_all_ho.append(q_imp_s)
            self.k_imp_s_all_ho.append(k_imp_s)
            self.v_imp_s_all_ho.append(v_imp_s)
            self.out_imp_s_all_ho.append(out_imp_s)
            self.up_imp_s_all_ho.append(up_imp_s)
            self.down_imp_s_all_ho.append(down_imp_s)
        elif current_type == 'general':
            # 存储常规数据的重要性分数(结构同上)
            self.q_imp_t_all_ge.append(q_imp_t)
            self.k_imp_t_all_ge.append(k_imp_t)
            self.v_imp_t_all_ge.append(v_imp_t)
            self.out_imp_t_all_ge.append(out_imp_t)
            self.up_imp_t_all_ge.append(up_imp_t)
            self.down_imp_t_all_ge.append(down_imp_t)

            self.q_imp_s_all_ge.append(q_imp_s)
            self.k_imp_s_all_ge.append(k_imp_s)
            self.v_imp_s_all_ge.append(v_imp_s)
            self.out_imp_s_all_ge.append(out_imp_s)
            self.up_imp_s_all_ge.append(up_imp_s)
            self.down_imp_s_all_ge.append(down_imp_s)

    def _get_important_score_CV(self, q, k, v, out, up, down, layer, name, param, q_imp, k_imp, v_imp, out_imp, up_imp,
                             down_imp):
        # 定义注意力层和FFN层的神经元选择数量
        attn_neurons_len = int(self.model.model_dim * self.args.select_ratio_CV)# 注意力层神经元数量
        up_len = int(self.model.feed_forward_dim * self.args.select_ratio_CV)# FFN上采样层神经元数量
    ...
    def reverse_finetune(self, current_model, setting, logger, data_loader, finetune_type):
        path = os.path.join(self.args.checkpoints, setting)  # 创建模型保存路径
        if not os.path.exists(path):
            os.makedirs(path)  # 如果路径不存在，则创建它
            
        # 使用常规数据集对模型进行训练
        opt = optim.AdamW(current_model.parameters(), lr=self.args.reverse_learning_rate)  # 定义优化器
        ...
        for epoch in range(num_epochs):  # 遍历训练的每个周期
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):  # 遍历数据加载器中的每个批次
                ...
                if self.args.use_amp:
                    ...
                    #current_model = self.update_model_grad_test(current_model, finetune_type)  # 完全反向
    ...
    def unfreeze_train(self, setting, logger):
        ...
        self.process_unfreeze(holiday_model, setting, logger, self.retrain_loader_ge)

        return self.model  # 返回模型实例以便在调用该方法后可以继续使用或保存模型
    
    def process_unfreeze(self,current_model,setting,logger,data_loader):
        path = os.path.join(self.args.checkpoints, setting)  # 创建模型保存路径
        if not os.path.exists(path):
            os.makedirs(path)  # 如果路径不存在，则创建它
            
        # 使用常规数据集对模型进行训练
        opt = optim.AdamW(current_model.parameters(), lr=self.args.unfreeze_learning_rate)  # 定义优化器
        ...
        self.test_model(current_model, self.debug_loader_ge, log_info='general after unfreeze train', log_name='after_reverse_general')
        self.test_model(current_model, self.test_loader, log_info='all test after unfreeze train', log_name='after_reverse_all')

    def test_train(self, setting, logger):
        # 对检测到的模式神经元进行微调，以提高模型在特定模式上的性能。
        self.setting = setting  # 保存当前的实验设置
        self.logger = logger  # 保存日志记录器
        ...
        # 如果有微调样本，则对节假日模型进行微调
        self.test_finetune(general_model, self.retrain_loader_ge, finetune_type='holiday')

        return self.model  # 返回模型实例以便在调用该方法后可以继续使用或保存模型

    def update_model_grad_test(self, model, finetune_type):
        # 更新模型梯度，根据选择的参数进行梯度更新。
        index_mappings = self.get_index_map()  # 获取索引映射，用于确定哪些神经元与特定模式相关
        for name, param in model.named_parameters():  # 遍历模型的所有参数
            match = re.search(r'layers_(\w)\.(\d+)', name)  # 使用正则表达式匹配参数名中的层类型和层号
            if match:
                ...
                for param_name, finetune_indices in layer_indices.items():  # 遍历当前层类型的参数名和微调索引
                    if param_name in name:  # 如果当前参数名包含在映射的参数名中
                        tune_index = finetune_indices[finetune_type][layer_num]  # 获取与微调类型和层号对应的神经元索引
                        mask = torch.zeros(param.size(0), dtype=torch.bool, device=param.device)  # 创建一个与参数大小相同的掩码，默认值为 False
                        mask[list(tune_index)] = True  # 将与微调相关的神经元索引位置设置为 True
                        param.grad[mask] = 0  # 将不需要更新的神经元梯度置零
                        param_matched = True  # 标志设置为 True，表示找到了匹配的参数
                        break
        return model  # 返回更新梯度后的模型
    ...
    def test_finetune(self, current_model, data_loader, finetune_type):
        # 对模型进行微调，使用特定数据集更新模型参数。
        opt = optim.AdamW(current_model.parameters(), lr=self.args.test_learning_rate)  # 定义优化器，使用AdamW算法和指定的学习率
        ...
        for epoch in range(self.args.finetune_epochs):  # 遍历微调的每个周期
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(data_loader):  # 遍历数据加载器中的每个批次
                ...
                if self.args.use_amp:
                    ...
                    amp_scaler.unscale_(opt)  # 反缩放梯度
                    #current_model = self.update_model_grad_test(current_model, finetune_type)  # 完全反向
                    current_model = self.update_model_grad(current_model, finetune_type)  # 部分反向
                    amp_scaler.step(opt)  # 更新优化器的参数
                    amp_scaler.update()  # 更新GradScaler对象的状态
                else:
                    curr_loss.backward()  # 反向传播计算梯度
    ...
    def reverse_train(self, setting, logger):
        ...
        self.reverse_finetune(holiday_model, setting, logger, self.retrain_loader_ge, finetune_type='general')
